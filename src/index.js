...
LS_CACHE_FILES.prototype = {
    ...
    init: function () {
        if (this.__LS__MAP.list && helper.isArray(this.__LS__MAP.list)) {
            this.__LS__MAP.list.map(item => {
                const prefixPath = helper.domUtils.PREFIX + item.url
     ...
     * @param {*} version 
     */
    checkFileAndCache: function(url,version) {
     ...
     * @param {*} version 
     */
    loadFileAndCache: function(url,version) {
        const prefixPath = helper.domUtils.PREFIX + url
        helper.domUtils.loadFile(url).then(res => {
